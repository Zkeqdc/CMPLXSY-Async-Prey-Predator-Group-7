...
to setup
  ...
  setup-people
  setup-globals
  if vaccine = "Astra Zeneca (70.4%)" [
    set vaccine-multiplier 1.704
  ]
  if vaccine = "Sinovac (51%)" [
    set vaccine-multiplier 1.51
  ]
  if vaccine = "Sputnik (91.6%)" [
    set vaccine-multiplier 1.916
  ]
  if vaccine = "Janssen (66.9%)" [
    set vaccine-multiplier 1.669
  ]
  if vaccine = "Bharat Biotech (80.6%)" [
    set vaccine-multiplier 1.806
  ]
  if vaccine = "Pfizer (95%)" [
    set vaccine-multiplier 1.95
  ]
  if vaccine = "Moderna (94.1%)" [
...
to infect ;; can be implemeted with either eat-sheep from Predation or infect from epiDem Basic
  let nearby-uninfected (turtles-on neighbors)
    with [ not infected? and susceptible?]
...
to get-vaccinated ;; Chase function in Disease Solo model
  if pcolor = cyan
    [ set color blue set vaccinated? true ]
end
...
422
62
*Efficacy rate of each available vaccine
11
0.0
1
...
"Susceptible" 1.0 0 -16777216 true "" "plot count turtles with [ not infected? ]"
