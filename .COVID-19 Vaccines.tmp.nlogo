...
to setup-global
  ...
  if vaccine-brand = "Sinovac (51%)" [
    set vaccine-multiplier 1.51
  ]
  if vaccine-brand = "Sputnik (91.6%)" [
    set vaccine-multiplier 1.916
  ]
  if vaccine-brand = "Janssen (66.9%)" [
    set vaccine-multiplier 1.669
  ]
  if vaccine-brand = "Bharat Biotech (80.6%)" [
    set vaccine-multiplier 1.806
  ]
  if vaccine-brand = "Pfizer (95%)" [
    set vaccine-multiplier 1.95
  ]
  if vaccine-brand = "Moderna (94.1%)" [
    set vaccine-multiplier 1.941
  ]
end
...
to assign-color  ;; turtle procedure
  ...
  if not infected? [
    if not cured? [
      if vaccinated? [ set color blue ]
    ]
  ]
  if cured? and vaccinated?
    [ set color cyan ]
end
...
;; Whenever susceptible turtles walk over the vaccination centers, there's only a certain chance that they'll get vaccinated upon
;; contact with the vaccination center patch
to get-vaccinated
  if pcolor = cyan [
    if not infected? and not vaccinated? [
      if random-float 100 < average-vaccination-tendency [
        set vaccinated? true
        set immunity immunity * vaccine-multiplier
      ]
    ]
  ]
end
...
to display-immunity
  ...
  if show-immunity? [
    ask turtles [ set label round immunity ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
263
Unvaccinated Recovery
count turtles with [ cured? and not vaccinated? ]
17
1
11
...
313
Vaccinated Recovery
count turtles with [ cured? and vaccinated? ]
17
1
11
...
"" ""
PENS
"Vaccinated" 1.0 0 -11221820 true "" "plot count turtles with [ cured? and vaccinated? ]"
"Susceptible" 1.0 0 -16777216 true "" "plot count turtles with [ susceptible? ]"
"Recovered" 1.0 0 -10899396 true "" "plot count turtles with [ cured? and not vaccinated? ]"
...
363
Vaccinated%
(count turtles with [ vaccinated? ] / initial-people)
17
1
11
...
"" ""
PENS
"% Vaccinated Recovery" 1.0 0 -11221820 true "" "plot (((count turtles with [ cured? and vaccinated? ]) / initial-people) * 100)"
"% Unvaccinated Recovery" 1.0 0 -10899396 true "" "plot (((count turtles with [ cured? and not vaccinated? ]) / initial-people) * 100)"
"% Unvaccinated Infection" 1.0 0 -2674135 true "" "plot (((count turtles with [ infected? and not vaccinated? ]) / initial-people) * 100)"
"% Vaccinated Infection" 1.0 0 -955883 true "" "plot (((count turtles with [ infected? and vaccinated? ]) / initial-people) * 100)"
